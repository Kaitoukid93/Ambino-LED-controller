...
namespace adrilight
{
    public static class MatrixFrame
    {
            ...
            if (orientation == Orientation.HZ)
            {
                for (int y = startY; y != termY; y += incY)
                {
                    for (int x = startX; x != termX; x += incX)
                    {
                        ...
                        orderedFrame[index * 3] = MatrixFrame.Frame[i].B;
                        orderedFrame[index * 3 + 1] = MatrixFrame.Frame[i].G;
                        orderedFrame[index * 3 + 2] = MatrixFrame.Frame[i].R;
                        index++;
                    }
                }
            }
        ...
        //ambilation assistant//
        public static void DrawRectangles(Int32 width, Int32 height, Canvas playground)
        {
            ...
            for (int j = 0; j < width; j++)
            {
                for (int i = 0; i < height; i++)
                {
                    ...
                    System.Windows.Shapes.Rectangle rectangle = new System.Windows.Shapes.Rectangle {
                        Width = playground.ActualWidth / width,
                        Height = playground.ActualWidth / width,
                    };

                    rectangle.Stroke = (SolidColorBrush)new BrushConverter().ConvertFromString("#FF7344C3");
                    rectangle.Fill = (SolidColorBrush)new BrushConverter().ConvertFromString("#FFFFFFFF");

                    //pix[i, j] = width * i + j;
                    playground.Children.Add(rectangle);

                    Canvas.SetLeft(rectangle, j * (playground.ActualWidth / width + 3));
                    Canvas.SetTop(rectangle, i * (playground.ActualWidth / width + 3));
    ...
    public static void SetRectanglesOrder(LEDOrder LEDorder, Canvas playground)
        {
            ...
            order.Text = Convert.ToString(LEDorder.pixelcounter);
            playground.Children.Add(order);
            order.Foreground = (SolidColorBrush)new BrushConverter().ConvertFromString("#FF7344C3");
            Canvas.SetLeft(order, LEDorder.row * (playground.ActualWidth / Width + 3) + 3);
            Canvas.SetTop(order, LEDorder.column * (playground.ActualWidth / Width + 3) + 3);

            


            //todo: get pixel order and draw it to matrix rectangle//
        }

        public static void SaveRectanglesOrder(LEDOrder LEDorder, Canvas playground)
        {
