...
{
    public class DeviceRainbow : IRainbow,IDisposable
    {
        ...
        public DeviceRainbow(DeviceInfoDTO device, ISpotSet spotSet,  LightingViewModel viewViewModel, SettingInfoDTO setting)
        {
        ...
        private void RefreshColorState()
        {
            ...
            var shouldBeRunning = deviceInfo.LightingMode == "Sáng theo màn hình";//UserSettings.TransferActive && UserSettings.SelectedEffect == 1;
            ...
            else if (!isRunning && shouldBeRunning)
            {
                ...
                var thread = new Thread(() => Run(_cancellationTokenSource.Token)) {
                    ...
                    Name = "DeviceRainbowColorCreator"
                };
                thread.Start();
            }
        }
...

        {
            ...
            try
            {
                ...
                while (!token.IsCancellationRequested)
                {
                    ...
                    int paletteSource = deviceInfo.Palette;//UserSettings.SelectedPalette;
                    var numLED =(deviceInfo.SpotsX - 1) * 2 + (deviceInfo.SpotsY - 1) * 2;
                    var colorOutput = new OpenRGB.NET.Models.Color[numLED];



                    bool isPreviewRunning = deviceInfo.LightingMode == "Sáng theo màn hình"; //(SettingsViewModel.IsSettingsWindowOpen && UserSettings.SelectedEffect == 1);
                    if (isPreviewRunning)
                    {
